...
%      end
    ...
    if observe_mode ==6 || observe_mode==7
        if Param.UpdateCModelParam == 1
            ...
            if ~isempty(p0)
                % lower and upper bound (disperssion Shift Ck Dk)
                lower_bound=[1e-3*ones(1,length(c_fill_ind))  1e-3*ones(1,length(d_fill_ind))];
                upper_bound=[1e2*ones(1,length(c_fill_ind))   1e2*ones(1,length(d_fill_ind))];
                % call optimization function
                %options = optimoptions('lsqnonlin','Display','off','DiffMaxChange',100,'MaxIter',1000);
%                 for s=-50:50
%                     ff(s+51)=PointProcessParamCD(-2+0.1*s);
%                     xx(s+51)=-2+0.1*s;
%                 end
                [p_opt,temp]   = fminsearchbnd(@PointProcessParamCD,p0,lower_bound,upper_bound);
                MaxO = -temp;
                disp('PointProcess');
                % update param
                Ck(c_fill_ind) = p_opt(1:length(c_fill_ind));
    ...
    if DISTR(2)==1
        if Param.UpdateDModelParam == 1
            ...
            if ~isempty([find(Param.dLinkUpdate)  find(MFk)])
                ...
                % define bounds
                lower_bound=-1e1*ones(1,length(p0));
                upper_bound= 1e1*ones(1,length(p0));
                % call optimization function
                %options = optimoptions('fmincon','Display','off','DiffMaxChange',100,'MaxIter',1000,'Algorithm','interior-point');
                %% Yalda: I only added this condition:DISTR(1)==3
                if DISTR(1)== 3
                    [p_opt,temp] = fminsearchbnd(@BernoulliPointProcessParam,p0,lower_bound,upper_bound);
                else
                    [p_opt,temp] = fminsearchbnd(@BernoulliParam,p0,lower_bound,upper_bound);
                end
%                 for s=-200:200
%                      t =p_opt;t(1)=s;
%                      ff(s+201)=BernoulliPointProcessParam(t);
%                  end
    ...
    if MaxH + MaxO + MaxB > 0.95 * Prv_ML
        ...
        disp(['ML did not increase, EM loop stopped'])
        break;
    end
...
%% Yalda:
% Bernoulli/PointProcess Paramater Update Function
    function f= BernoulliPointProcessParam(p)
        % replace param
        ek(e_fill_ind) = p(1:length(e_fill_ind));
        fk(f_fill_ind) = p(length(e_fill_ind)+1:end);
        % function calculation
...
% PointProcess Paramater Estimation
    function f = PointProcessParamCD(p)
        yk = Yn;
        % ctk
        ck(c_fill_ind) = p(1:length(c_fill_ind));
        % dtk - parameters linked to Input
        ...
        for l =1:length(val_ind)
            ...
            f = f - yk(t)*log(dt) - yk(t)*ey + dt*exp(ey+0.5*sy);
        end
    end
